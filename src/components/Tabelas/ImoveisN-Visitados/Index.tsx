...
type BackendRow = {
  registro_de_campo_id?: number
  area_de_visita?: { setor?: string; logadouro?: string; bairro?: string; municipio?: string }
...
function Index() {
  ...
  const [filters, setFilters] = useState<Record<string, string[]>>({ setor: [], tipo: [], status: []})
  ...
  const normalize = (r: BackendRow): RowData => {
    ...
    return {
      ...
      bairro: safe(r.area_de_visita?.bairro),
         municipio: safe(r.area_de_visita?.municipio),
   ...
   { accessorKey: "municipio", header: "Município" },
    ...
    { accessorKey: "tipo", header: "Tipo de Imóvel" },
    {
      accessorKey: "status", header: "Status", cell: ({ getValue }) => {
        ...
        const cls =
          s === "Não visitado" ? "bg-red-100 text-red-700 border border-red-700" :
